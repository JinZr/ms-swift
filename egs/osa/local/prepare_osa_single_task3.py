...
def write_split(
    ...
    with path.open("w", encoding="utf-8") as fout:
        for idx in indices:
            ...
            patient_info = f"这是一位年龄 {age_val} 岁，体重 {weight_val} kg，BMI {bmi_val}，颈围 {neck_val}cm 的患者"
                ...
                for col_idx, col in enumerate(response_cols):
                    ...
                    if col == "AHI":
                        try:
                            ...
                            pass  # leave col_val as-is if it can't be parsed
                    ...
                    if tail.startswith("请听录音并"):
                        tail = tail[len("请听录音并") :]
                    # 构造自然语言查询前缀
                    natural_prefix = f"{patient_info}, 请听说话人{ai_phrase}的录音，并{tail}"

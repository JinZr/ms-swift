...
• The model now performs *binary* OSA classification only; expected responses are the literal tokens <SEVERE> and <NON_SEVERE>.
"""
...
def write_split(
    ...
    with path.open("w", encoding="utf-8") as fout:
        for idx in indices:
            ...
            # Iterate over pre‑defined (seated, supine) pairs for the same content
            for ai1, ai2 in AUDIO_INDEX_PAIRS:
                # Skip this pair if either index was disabled on the CLI
                ...
                if not wav1.exists() or not wav2.exists():
                    continue  # require BOTH recordings
                ...
                for col_idx, col in enumerate(response_cols):
                    ...
                    if col == "AHI":
                        try:
                            ahi = float(col_val)
                            # Binary classification label tokens
                            col_val = "<NON_SEVERE>" if ahi < 30 else "<SEVERE>"
                        except ValueError:
                            # Fallback: treat invalid/missing AHI as non‑severe
                            col_val = "<NON_SEVERE>"
